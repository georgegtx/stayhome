  ...
  render() {
    ...
    if (showAll || this.props.showTransmissionProbabilitySlider) {
      transmissionProbabilitySlider =
          this.renderSlider("Ρυθμός μετάδοσης", this.state.transmissionProbability,
    ...
    if (showAll || this.props.showHospitalCapacitySlider) {
      ...
      hospitalCapacitySlider =
          this.renderSlider("Χωρητικότητα νοσοκομείων", this.state.hospitalCapacityPct,
    ...
    if (showAll || this.props.showTravelRadiusSlider) {
      travelRadiusSlider =
          this.renderSlider("Ακτίνα μετακινήσεων", this.state.travelRadius,
    ...
    if (showAll || this.props.showPersonHoursSlider) {
      personHoursSlider =
          this.renderSlider("Συναθροίσεις ανα μέρα", this.state.personHours,
    ...
    if (showAll || this.props.showDaysPerStateControls) {
      daysIncubatingSlider =
          this.renderSlider("Διάρκεια επώασης (μέρες)", this.state.daysIncubating,
    ...
    if (showAll || this.props.showDaysPerStateControls) {
      daysSymptomaticSlider =
          this.renderSlider("Διάρκεια με συμπτώματα (μέρες)", this.state.daysSymptomatic,
    ...
    if (showAll || this.props.showChanceOfIsolationAfterSymptomsSlider) {
      chanceOfIsolationAfterSymptomsSlider =
          this.renderSlider("Ρυθμός αυτο-περιορισμού", this.state.chanceOfIsolationAfterSymptoms,
    ...
    if (showAll || this.props.showDecreaseInEncountersAfterSymptomsSlider) {
      decreaseInEncountersAfterSymptomsSlider =
          this.renderSlider("Αυστηρότητα αυτο-περιορισμού", this.state.decreaseInEncountersAfterSymptoms,
    ...
    if (showAll || this.props.showDeathRateSlider) {
      let sliderName = "Ρυθμός θνησιμότητας";
      if (this.state.hospitalCapacityPct > -1) {
        sliderName = "Ρυθμός θνησιμότητας εισαγωγών";
      }
    ...
    if (showAll || this.props.showPlaybackControls) {
      let newNetworkButton = <WidgetButton onClick={() => {this.setState({playing: false}); this.generate(true); this.forceUpdate();} } >Επαναφορά</WidgetButton>;
      ...
      let stepButton = <WidgetButton onClick={() => {this.simulateStep(); this.setState({playing: false}); } } >Βήμα</WidgetButton>;
    ...
    if (this.props.showProTip) {
      protip = (
          <div style={{color: '#666', fontSize: '12pt', marginTop: '1em'}}>👆 Tip: Μπορείς να προσαρμόσεις τις τιμές καθώς τρέχει η προσομοίωση.</div>
      );
    }
    ...
    return (
        <div className="widget-container" style={{display: 'flex', flexDirection: 'column', alignItems: 'center'}}>
          ...
          {playbackControls}
          <div style={{height: "1em"}}> Μέρα: {this.state.totalDays}</div>
